...
class Telnet < EM::Connection
    ...
    def initialize
        ...
        if File.exists?(SYSTEM + "/iniquity.ini")
            system = IniFile.load(SYSTEM+ "/iniquity.ini")
            script = IO.popen(["ruby", system["profile"]["script"]])
            #listing = io.read
            #io.close
            #raise "it failed!" unless $?.exitstatus == 0
            #EM.next_tick { send_data listing.force_encoding(Encoding::IBM437).encode(Encoding::UTF_8) }
            script.readlines.each do |line|
                EM.next_tick { send_data line.force_encoding(Encoding::IBM437).encode(Encoding::UTF_8) }
            end
        end
...
class Http < EM::HttpServer::Server
    ...
    def process_http_request
          ...
          response.content = 'Iniquity BBS :: Build 0.0.20'
    ...
    CONFIG.each_section do |section|
        ...
        if section == "websocket"
            if CONFIG[section]["enabled"] == "yes"
                ...
                puts "iniquity - Websocket service started."
                EM.start_server "0.0.0.0", CONFIG[section]["port"], Websocket
            end
        end
    end
end
