 ...
 */
const BORDER_CHARS: Record<IQFrameBorderStyle, {
    topLeft: string
    topRight: string
    bottomLeft: string
    bottomRight: string
...
export class IQFrame {
     ...
     */
    separator(char: string = '\xC4', color?: string): IQFrame {  // CP437: ─
        const contentWidth = this.width - 2
        const line = color 
            ? ANSI.color(color) + char.repeat(contentWidth) + ANSI.reset()
            : char.repeat(contentWidth)
