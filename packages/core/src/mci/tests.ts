...
function runTests() {
    ...
    test("createDefaultMCIContext creates valid context", () => {
        ...
        return ctx.user !== undefined && ctx.system !== undefined && ctx.bbs !== undefined && ctx.terminal !== undefined
    })
    ...
    test("AtCodeProcessor returns terminal dimensions", () => {
        ...
        return processor.getValue("COLS", ctx) === "132" && processor.getValue("ROWS", ctx) === "50"
    })
    ...
    test("ControlCodeProcessor handles delay codes", () => {
        ...
        return de?.type === "delay" && de.ms === 500 && dm?.type === "delay" && dm.ms === 1000 && dl?.type === "delay" && dl.ms === 2000
    })
    ...
    test("ControlCodeProcessor handles cursor movement", () => {
        ...
        return up?.type === "ansi" && up.sequence === `${ESC}[5A` && down?.type === "ansi" && down.sequence === `${ESC}[3B`
    })
    ...
    test("PositionMarkerProcessor processes ^X codes in text", () => {
        ...
        return result.result === "Hello  World " && result.markersFound.includes(1) && result.markersFound.includes(2)
    })

    test("createPositionMarker creates valid marker string", () => {
        return createPositionMarker(1) === "^X01" && createPositionMarker(99) === "^X99"
    })
    ...
    test("findAllPositionMarkers finds all unique markers", () => {
        ...
        return markers.length === 3 && markers.includes(1) && markers.includes(5) && markers.includes(99)
    })
    ...
    test("parseFormatModifier parses pipe format", () => {
        ...
        return result.code === "UN" && result.modifier === "L" && result.width === 20
    })

    test("parseFormatModifier parses hyphen format", () => {
        const result = parseFormatModifier("BBS-R30")
        return result.code === "BBS" && result.modifier === "R" && result.width === 30
    })
    ...
    test("leftJustify pads correctly", () => {
        return leftJustify("Hi", 5) === "Hi   " && leftJustify("Hello World", 5) === "Hello"
    })

    test("rightJustify pads correctly", () => {
        return rightJustify("Hi", 5) === "   Hi" && rightJustify("Hello World", 5) === "Hello"
    })

    test("center pads correctly", () => {
        return center("Hi", 6) === "  Hi  " && center("A", 5) === "  A  "
    })

    test("zeroPad pads numbers correctly", () => {
        return zeroPad("42", 5) === "00042" && zeroPad("123", 3) === "123"
    })

    test("addThousandsSeparator formats numbers", () => {
        return addThousandsSeparator("1234567") === "1,234,567" && addThousandsSeparator("1234.56") === "1,234.56"
    })
    ...
    test("MCIProcessor handles position markers", () => {
        ...
        return result.markersFound.includes(1) && !result.text.includes("^X01")
    })
