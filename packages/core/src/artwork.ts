...
export class Artwork {
    ...
    private parseSauce(buffer: Buffer): SAUCEInfo | null {
        ...
        const title = buffer
            .toString("ascii", offset + 7, offset + 42)
            .trim()
            .replace(/\0/g, "")
        const author = buffer
            .toString("ascii", offset + 42, offset + 62)
            .trim()
            .replace(/\0/g, "")
        const group = buffer
            .toString("ascii", offset + 62, offset + 82)
            .trim()
            .replace(/\0/g, "")
        ...
        const fontName = buffer
            .toString("ascii", offset + 106, offset + 128)
            .trim()
            .replace(/\0/g, "")
            ...
            if (commentStart >= 0) {
                ...
                if (commentId === "COMNT") {
                    for (let i = 0; i < commentLines; i++) {
                        ...
                        const line = buffer
                            .toString("ascii", lineStart, lineStart + 64)
                            .trim()
                            .replace(/\0/g, "")
